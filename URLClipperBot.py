...
async def URLShorten(update: Update, context: CallbackContext) -> None:
    ...
    message = await context.bot.send_message(
        text="_fetching url..._", chat_id=chatID, parse_mode="Markdown"
    )
    ...
    data = (
        requests.get(
            "http://cutt.ly/api/api.php?key={}&short={}".format(API_KEY, toShorten)
        )
        .text.replace('"', "")
        .replace("\\", "")
        .replace("}", "")
        .replace("{", "")
        .replace("url:status:7,", "")
    )
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    if numUses == 0:
        await update.message.reply_text(
            f"Hello {userName}, welcome to URL Clipper Bot.\nIf you need any help, feel "
...
async def myURLs(update: Update, context: CallbackContext) -> None:
    ...
    if uses == 0:
        ...
        for i in range(0, len(urlData), 4):
            ...
            title = (
                urlData[i + 3].replace("title:", "").replace("'", "").replace("}", "")
            )
